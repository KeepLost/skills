...
// These constants are intentionally not configurable. The companion must never
// listen on a routable interface or advertise a non-loopback URL.
const HOST = '127.0.0.1';
const URL_HOST = 'localhost';
const SESSION_DIR = process.env.BRAINSTORM_DIR || '/tmp/brainstorm';
const CONTENT_DIR = path.join(SESSION_DIR, 'content');
const STATE_DIR = path.join(SESSION_DIR, 'state');
const BRAINSTORMING_VERSION = readPackageVersion();
const TELEMETRY_DISABLE_ENV_VARS = [
  'BRAINSTORMING_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const BRAINSTORMING_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
let ownerPid = process.env.BRAINSTORM_OWNER_PID ? Number(process.env.BRAINSTORM_OWNER_PID) : null;

// Per-session secret key. The companion is reachable by local browser tabs, so
// the key authenticates the intended client and helps defeat DNS rebinding. It
// rides the served URL as ?key= and is mirrored into a cookie on first load so
// same-origin subresources and the WebSocket carry it for free.
...
function brandMarkup() {
  const version = escapeHtmlText(BRAINSTORMING_VERSION);
  const text = BRAINSTORMING_TELEMETRY_DISABLED
    ? 'Brainstorming Companion'
    : 'Brainstorming Companion v' + version;
...
function hasLoopbackHostHeader(req) {
  ...
  return typeof host === 'string' && /^(localhost|127\.0\.0\.1)(:\d{1,5})?$/i.test(host);
}
...
function handleRequest(req, res) {
  if (!hasLoopbackHostHeader(req) || !isAuthorized(req)) {
...
function handleUpgrade(req, socket) {
  if (!hasLoopbackHostHeader(req) || !isAuthorized(req) || !isAllowedWebSocketOrigin(req)) {
    socket.destroy();
    return;
  }
...
// Best-effort: open the local browser the first time a screen is ready. The
// platform launcher receives the URL as an argv element and never invokes a
// shell or accepts an operator-supplied command.
