...
Browser-based visual brainstorming companion for showing mockups, diagrams, and options on the same machine.

## Local-Only Safety Boundary

The companion is a loopback-only tool. It always listens on `127.0.0.1` and prints a `localhost` URL. Do not proxy, tunnel, port-forward, container-publish, or otherwise expose its port. The launcher intentionally has no bind-host or display-host override. Keep the session key in the complete URL private even though the listener is local.

Served pages use a restrictive content policy: scripts, styles, media, and connections are limited to the local origin plus inline or embedded data needed for mockups. Put assets in `screen_dir` and reference them through `/files/`.
...
Save `screen_dir` and `state_dir` from the response. With `--open`, the local browser opens when you push the first screen. Share the complete loopback URL as a fallback.

**The URL contains a session key (`?key=…`).** The server rejects any request without it, so always give the user the **complete** URL from the `url` field. Never strip the query string or hand out a bare port. The key gates HTTP and WebSocket access so another local browser context cannot read screens or inject events. After the first load the browser remembers the key via a cookie, so reloads and `/files/*` assets work without repeating it.
...
If the environment reaps detached processes, use `--foreground` with its normal background-task mechanism. This changes process lifetime only; it never changes the loopback listener.
...
- **Use real local content when it matters** — prefer repository or user-provided
  images. Otherwise use a labeled local placeholder with the intended dimensions
  and aspect ratio.
